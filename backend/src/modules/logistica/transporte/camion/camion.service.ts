...
export class CamionService {
  ...
  findAllForTransporte(idTransporte: number) {
    return this.prisma.camion.findMany({ where: { id_transporte: idTransporte } });
  }

  create(idTransporte: number, dto: CreateCamionDto) {
    return this.prisma.camion.create({ data: { ...dto, id_transporte: idTransporte } });
  }
  ...
  async findOne(idTransporte: number, idCamion: number) {
    ...
    if (!camion) {
      throw new NotFoundException(`Camión ${idCamion} no encontrado en el transporte ${idTransporte}`);
    }
    return camion;
  }
  ...
  async update(idTransporte: number, idCamion: number, dto: UpdateCamionDto) {
    ...
    return this.prisma.camion.update({ where: { id_camion: idCamion }, data: dto });
  }
