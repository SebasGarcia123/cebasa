...
export class SectoresList implements OnInit {
  private readonly api = inject(SectoresApiService);
    ...
    this.api.list().subscribe({
      ...
      error: () => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'No se pudo cargar el listado de sectores',
        });
      },
    });
  }
  ...
  openEdit(sector: Sector): void {
    this.editingId.set(sector.id_sector);
    this.form.setValue({ nombreSector: sector.nombreSector });
    this.dialogVisible.set(true);
  }
    ...
    request$.subscribe({
      next: () => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Guardado',
          detail: 'El sector se guardó correctamente',
        });
        this.load();
      },
      error: () => {
  ...
  private remove(id: number): void {
    this.api.remove(id).subscribe({
      next: () => {
        this.messageService.add({ severity: 'success', summary: 'Eliminado', detail: 'Sector eliminado' });
        this.load();
      },
      error: () => {
        this.messageService.add({
