...
@Component({
  selector: 'app-tipo-impacto-list',
  imports: [ReactiveFormsModule, TableModule, ButtonModule, DialogModule, InputTextModule],
  templateUrl: './tipo-impacto-list.html',
  styleUrl: './tipo-impacto-list.scss',
})
export class TipoImpactoList implements OnInit {
  private readonly api = inject(TipoImpactoApiService);
  private readonly fb = inject(FormBuilder);
  private readonly confirmationService = inject(ConfirmationService);
  private readonly messageService = inject(MessageService);

  protected readonly items = signal<TipoImpacto[]>([]);
  ...
  openEdit(item: TipoImpacto): void {
    this.editingId.set(item.id_tipo_impacto);
    this.form.setValue({ descripcion: item.descripcion });
    this.dialogVisible.set(true);
  }
  ...
  confirmDelete(item: TipoImpacto): void {
    this.confirmationService.confirm({
      ...
      accept: () => this.remove(item.id_tipo_impacto),
    });
  }
