...
export class TipoProductoList implements OnInit {
  private readonly api = inject(TipoProductoApiService);
  private readonly fb = inject(FormBuilder);
  private readonly confirmationService = inject(ConfirmationService);
  private readonly messageService = inject(MessageService);

  protected readonly items = signal<TipoProducto[]>([]);
  ...
  openEdit(item: TipoProducto): void {
    this.editingId.set(item.id_tipo_producto);
  ...
  confirmDelete(item: TipoProducto): void {
    this.confirmationService.confirm({
      ...
      accept: () => this.remove(item.id_tipo_producto),
    });
  }
