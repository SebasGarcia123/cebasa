...
@Component({
  ...
  templateUrl: './naturaleza-list.html',
  styleUrl: './naturaleza-list.scss',
})
export class NaturalezaList implements OnInit {
  private readonly api = inject(NaturalezaApiService);
  private readonly fb = inject(FormBuilder);
  private readonly confirmationService = inject(ConfirmationService);
  private readonly messageService = inject(MessageService);

  protected readonly items = signal<Naturaleza[]>([]);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre_naturaleza: ['', Validators.required],
  });
  ...
  openEdit(item: Naturaleza): void {
    this.editingId.set(item.id_naturaleza);
    this.form.setValue({ nombre_naturaleza: item.nombre_naturaleza });
    this.dialogVisible.set(true);
  }
